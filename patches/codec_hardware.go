...
// HWCodecHLSCompatible returns a hardware accelerated codec for HLS if available
...
// HWCodecMP4Compatible returns a hardware accelerated codec for MP4 if available
...
// HWCodecWEBMCompatible returns a hardware accelerated codec for WebM if available
...
// HWDeviceInit prepends hardware device initialization arguments for the given codec.
// This is an exported wrapper around the internal hwDeviceInit method.
func (f *FFMpeg) HWDeviceInit(args Args, toCodec VideoCodec, fullhw bool) Args {
	return f.hwDeviceInit(args, toCodec, fullhw)
}

// HWFilterInit returns the initial video filter for hardware encoding.
// This is an exported wrapper around the internal hwFilterInit method.
func (f *FFMpeg) HWFilterInit(toCodec VideoCodec, fullhw bool) VideoFilter {
	return f.hwFilterInit(toCodec, fullhw)
}

// HWMaxResFilter returns a video filter with hardware scaling for the given codec.
// This is an exported wrapper around the internal hwMaxResFilter method.
func (f *FFMpeg) HWMaxResFilter(toCodec VideoCodec, vf *models.VideoFile, reqHeight int, fullhw bool) VideoFilter {
	return f.hwMaxResFilter(toCodec, vf, reqHeight, fullhw)
}

// HWCanFullHWTranscode tests if full hardware transcoding is possible for the given file.
// This is an exported wrapper around the internal hwCanFullHWTranscode method.
func (f *FFMpeg) HWCanFullHWTranscode(ctx context.Context, codec VideoCodec, vf *models.VideoFile, reqHeight int) bool {
